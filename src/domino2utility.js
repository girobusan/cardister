 ...
 * @returns {HTMLElement}
 */
export const ONE = (query, element = undefined) => (element || document).querySelector(query);
/**
 * @param {string} query 
 * @param {HTMLElement | Document} element 
 * @returns {HTMLElement[]}
 */
export const ALL = (query, element = undefined) => Array.from((element || document).querySelectorAll(query));
 ...
 */
 export function refreshDropdown(element, data, renderer) {
 ...
 */
export function eventToElementPixel(event, element) {
...
export function imageToRendering2D(image) {
 ...
 * @returns {boolean}
 */
export function arrayDiscard(array, value) {
 ...
 */
export function svg(tagName, attributes = {}, ...children) {
 ...
 * @param {string} source
 */
export async function htmlFromText(source) {
 ...
 * @param {string} text 
 */
export function textToBlob(text, type = "text/plain") {
 ...
 * @returns {Rect}
 */
export function boundRects(rects, fallback = { x: 0, y: 0, width: 0, height: 0 }) {
 ...
 * @param {number} end 
 */
export function insertText(original, insert, start, end) {
